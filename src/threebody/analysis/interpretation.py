...
def _make_segment(
    ...
    if chart == ChartType.CLOSE_ENCOUNTER and getattr(system, "body_count", None) == 3:
        ...
        if collision.levi_civita_equivalence_resolved:
            resolved_obligations = (
                *resolved_obligations,
                "numerically certify Levi-Civita inertial equivalence residual",
            )
...
def _model_for_chart(chart: ChartType) -> dict[str, str | float | tuple[str, ...]]:
    models: dict[ChartType, dict[str, str | float | tuple[str, ...]]] = {
        ...
        ChartType.CLOSE_ENCOUNTER: {
            ...
            "unresolved_obligations": (
                "derive near-collision residual bound for Levi-Civita regularized flow",
                "derive analytic equivalence theorem for regularized and inertial charts away from collision",
            ),
        },
