...
*   **Engine API 분리:** 배포명은 `threebody-engine`으로 조정되었고, 안정 API 표면은 `threebody_engine` 패키지에 추가되었습니다. 주요 entry point는 `integrate_reference_scenario`, `certify_jacobi_escape`, `certify_jacobi_escape_report`, `tune_jacobi_picard`, `build_hysteresis_markov_chain`, `validate_hysteresis_markov_chain`, `compare_hysteresis_markov_to_baseline`, `compare_hysteresis_markov_to_baseline_with_uncertainty`, `run_verification_report`입니다.
