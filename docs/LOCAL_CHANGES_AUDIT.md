...
*   **Malformed provenance hardening:** verifier는 이제 `build_provenance`나 `artifacts` 같은 중첩 JSON 필드가 객체가 아니어도 예외로 중단하지 않고 receipt의 개별 check 실패로 보고합니다. 또한 certificate와 manifest의 commit provenance는 양쪽 모두 비어 있지 않은 문자열이고 서로 같을 때만 통과합니다.
