...
*   **Poincare section 진단 추가:** `poincare_section_word_from_reports`와 `word_mode="poincare"`가 추가되어 명시적 단면 통과 사건으로 chart word를 만들 수 있습니다. 대표 Pages 시나리오에서는 section crossing 수가 아직 부족하므로 승격 게이트는 refined chart-word grammar를 유지하고, Poincare word는 JSON 인증서 안의 반증/진단 항목으로 노출합니다.
