...
*   **Random three-body prediction demo:** `threebody_engine.generate_random_three_body_case`, `solve_random_three_body_prediction_demo`, `threebody random-demo`가 추가되었습니다. 이 경로는 임의 seed의 비충돌 일반 삼체 초기조건을 생성하고 adaptive final state, deterministic ephemeris final row, compact target solver, empirical mean readout을 고정밀 reference integration과 비교해 point error, energy drift, close-approach gate를 통과해야만 success를 승격합니다.
...
*   **GitHub Pages content-switching navigation:** static site는 이제 왼쪽 fixed navigation rail로 우측 research content workspace를 같은 페이지 안에서 교체합니다. ThreeBody, Riemann, Collatz, Goldbach, twin-prime 패널과 GitHub repository 버튼을 제공하며 메뉴/패널 정의는 `threebody.ui.static_content`에 분리했습니다.
