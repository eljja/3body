...
*   **GitHub Pages floating navigation:** static site는 이제 왼쪽 fixed navigation rail을 제공합니다. target answer, progress map, public audit, engine upgrades, representative figures, promotion gates, build provenance로 즉시 이동하고 GitHub repository로 바로 열리는 버튼을 포함합니다.
