...
- The refined chart-word promotion gate is accompanied by a multi-coordinate Poincare sweep. The fixed hierarchy-perturbation section remains a falsification diagnostic, while the coordinate sweep searches common chart diagnostics, reports the best crossing-rich section, validates its Markov memory against both an independent-symbol baseline and a shuffled-symbol permutation control, and reports whether nearby section quantiles pass the same memory gates.
