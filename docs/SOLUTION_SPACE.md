...
- `threebody_engine.solve_three_body_target_positions(...)` answers the same target in the compact form most callers want first: target-time positions, target-time distribution, per-body answer rows, deterministic flow definition, probability push-forward definition, and core reliability diagnostics without the full ephemeris bundle.
