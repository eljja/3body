...
- `threebody_engine.solve_three_body_target_positions(...)` answers the same target in the compact form most callers want first: target-time positions, target-time distribution, empirical sampling-quality diagnostics, a table-ready per-body coordinate/probability summary, center-of-mass-frame coordinates, pairwise target geometry, relative uncertainty strength, recommended readout, deterministic flow definition, probability push-forward definition, reproducibility certificate, and core reliability diagnostics without the full ephemeris bundle.
