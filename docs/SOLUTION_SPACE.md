...
**Paper status.** This is a design-space and prioritization document. It can
justify why several research branches remain active, but it should not be cited
as proof of any branch law without the corresponding validation document.

**논문용 상태.** 이 문서는 연구 방향과 우선순위를 정리한 설계공간 문서이다.
각 branch law의 증거로 인용하려면 해당 검증 문서와 gate 결과를 함께 제시해야
한다.
...
- `threebody_engine.solve_random_three_body_prediction_demo(...)` is the public success harness for the original "random three-body at time t" goal: it generates a reproducible non-collisional random initial state, computes the target-time coordinates through the point solver, ephemeris, compact target solver, and empirical uncertainty push-forward, then compares those readouts against a stricter high-precision reference integration with explicit error, invariant-drift, and close-approach gates.
- `threebody_engine.assess_three_body_global_closed_form_claim(...)` answers the new closed-form research question in a deliberately narrow way: it does not promote a finite elementary global formula, but it does test whether a supplied initial state passes the current gates for a Sundman-style regularized convergent-series contract. The returned certificate records pair distances, angular momentum, center-of-mass diagnostics, readiness checks, and the proof obligations still required before an effective global series solver can be claimed.
- `threebody_engine.solve_three_body_prediction_problem(...)` answers the full operational version in one call: final positions, deterministic ephemeris, linearized Gaussian ephemeris, time-resolved empirical distribution, Gaussian-vs-empirical ephemeris comparison, forecast-horizon diagnostics, recommended interpretation mode, a report-ready `prediction_summary`, and a machine-readable `mathematical_statement`. The summary promotes the result as `target-position-and-distribution`, `distributional-target-position`, `deterministic-target-position`, or `unresolved-target-position`; the mathematical statement records the Newtonian equations, flow-map readout, probability push-forward, covariance formula, and per-body target-time claim rows. Its default uncertainty model preserves center-of-mass position and velocity in both the empirical ensemble and linearized covariance; if `initial_state_covariance` is supplied, that full covariance is shared by the empirical, linearized, horizon, and report paths. If callers provide nonuniform `target_times`, the deterministic, linearized, empirical, and comparison ephemerides now share that exact observation grid and report alignment diagnostics.
- `threebody_engine.solve_three_body_target_positions(...)` answers the same target in the compact form most callers want first: target-time positions, target-time distribution, empirical sampling-quality diagnostics, finite-time sensitivity budget, a table-ready per-body coordinate/probability summary, center-of-mass-frame coordinates, pairwise target geometry, relative uncertainty strength, target readout decision, deterministic flow definition, probability push-forward definition, reproducibility certificate, certificate validation API, and core reliability diagnostics without the full ephemeris bundle.
- `threebody_engine.predict_three_body_positions(...)` answers: given three masses, finite non-collisional initial positions, initial velocities, and a target time, where are the three bodies if the finite-time diagnostics remain admissible? It integrates the declared Newtonian initial-value problem and returns final positions, velocities, solver metadata, energy/momentum/angular-momentum drift diagnostics, and minimum pair-distance close-approach diagnostics. If close-approach diagnostics recommend regularization, the result should be reported as limited or unresolved rather than as a paper-level point-position claim.
