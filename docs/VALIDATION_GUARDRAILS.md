...
- Hysteresis-width collapse also fails in the theorem harness, so re-entry should not be forced into a scalar boundary model. The next admissible target is a chart-sequence/return-map model.
- `Chart-Word Grammar` currently passes nearest-neighbor held-out word stability in the flyby theorem harness, but held-out word diversity is too weak. It cannot be promoted until it survives alphabet refinement, classifier perturbations, and a nontrivial return-map construction.
