...
- Periodic-neighborhood interpretation now emits both a finite-difference flow-map monodromy/shadowing proxy and, for declared periodic candidates such as figure-eight, a variational state-transition certificate. The figure-eight benchmark now checks closure, `det(Phi) ~= 1`, mass-weighted symplectic-form preservation `Phi^T Omega Phi ~= Omega`, reciprocal multiplier pairing, a Floquet-style linear-stability proxy, and a predeclared Jacobian-step convergence guardrail. This is stronger than visual recurrence, but still not a general periodic-orbit theorem.
