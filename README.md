...
`answer_three_body_problem` is the direct theorem-facing wrapper for the same target. It returns `theorem_answer`, `position_answer`, `distribution_answer`, and `decision_protocol` so the original objective is explicit: for admissible finite data, compute `r_i(t) = Pi_{r_i} Phi_t(x0)` when the target time is diagnostically resolved, or report `mu_t = (Phi_t)_# mu_0` when initial uncertainty dominates. 한국어로는 “임의의 삼체 초기조건과 목표시간이 주어졌을 때 각 좌표를 말할 수 있으면 좌표를 말하고, 혼돈/불확실성이 커지면 목표시간 확률분포를 말한다”는 계층이다. 이 계층은 전역 초등 닫힌 해를 주장하지 않고, 유한시간 뉴턴 흐름과 push-forward 확률법칙으로 방어 가능한 답만 승격한다.
