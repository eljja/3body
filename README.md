...
The same verifier also checks artifact identity, the declared manifest hash algorithm, and publication-pipeline links, so a receipt fails if `certificate.json` no longer declares the static research certificate, `manifest.json` no longer declares the static-site manifest with `hash_algorithm: sha256`, or the certificate stops pointing back to the published certificate and manifest filenames. Missing local files, fetch failures, missing direct byte inputs, invalid JSON, and malformed nested provenance/artifact sections are reported as failed receipt checks, and commit provenance only passes when both artifacts declare the same non-empty commit string.
