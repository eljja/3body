...
The same verifier also checks artifact identity, the declared manifest hash algorithm, index discoverability links, and publication-pipeline links, so a receipt fails if `certificate.json` no longer declares the static research certificate, `manifest.json` no longer declares the static-site manifest with `hash_algorithm: sha256`, `index.html` stops linking the public certificate/manifest/favicon artifacts, or the certificate stops pointing back to the published certificate and manifest filenames. Receipts include `verification_schema_features` and `verification_schema_features_sha256`; callers can repeat `--require-feature <name>` or pass `--require-feature-set-sha256 <digest>` to make the receipt fail unless the verifier advertises the expected capabilities. Missing local files, fetch failures, missing direct byte inputs, invalid JSON, and malformed nested provenance/artifact sections are reported as failed receipt checks, and commit provenance only passes when both artifacts declare the same non-empty commit string.
